...
<body>
		...
		<nav>
			...
			<ul class="menubar">
				...
				<li><label for="btns"><input class="btns selected"
						type="button" value="신고 목록"
						onClick="location.href='select.report'"></label></li>
				<li><label for="btns"><input class="btns" type="button"
						value="회원정보 조회/수정" onClick="location.href='select.user'"></label></li>
			</ul>
		</nav>
					...
					<div id="paging-wrap">
						...
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<c:choose>
								...
								<c:otherwise>
									<a href="/day01/select.report?page=${i} " class="paging change-page"><c:out value="${i}" /></a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${next}">
	...
	<form action="${pageContext.request.contextPath}/select.report" name="page-form">
...
<script src="${pageContext.request.contextPath}/static/js/admin/report.js"></script>
